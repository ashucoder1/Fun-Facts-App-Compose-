...
fun UserInputScreen(userInputViewModel: UserInputViewModel,
    ...
    Surface(
        ...
        Column(
            ...
            TopBar("Hi there  \uD83D\uDE0A")  //emoji unicode
            TextComponent(
                textValue = "Let's Learn About You !",
                textSize = 24.sp)
            Spacer(modifier = Modifier.size(20.dp))
            TextComponent(textValue = "This app will prepare a details page" +
            ...
            if(!userInputViewModel.uiState.value.nameEntered.isNullOrEmpty()
                ...
                ButtonComponent(
                    goToDetailsScreen = {
                        println("==ComingHere")
                        println("==Coming${userInputViewModel.uiState.value.nameEntered} and ${userInputViewModel.uiState.value.animalSelected}")
                        showWelcomeScreen(
                            Pair(
...
fun UserInputScreenPreview(){
    val samplePair = Pair("John Doe", "Cat") // Sample data for the Pair
    UserInputScreen(userInputViewModel = UserInputViewModel()) { valuePair ->
        // This lambda simulates the behavior of showWelcomeScreen
        // You can access the values from samplePair via valuePair
        // For example, to print the values:
        println("Name: ${valuePair.first}, Animal: ${valuePair.second}")
    }
}
